...
def build_base_model(input_shape=None, Attention_block_1=None,Attention_block_2=None, Specification=None, initializer='random_uniform'):
    Inputs = Input(shape=input_shape)
    # x = Resizing(height=input_shape[1][0],width=input_shape[1][1],interpolation="bicubic")(Inputs)
    x = Inputs

    for Spec in Specification:
        # 0-Operator, 1-exp_size, 2-filters, 3-kernel_size, 4-strides, 5-padding, 6-activation, 7-Attention_block_1 8-Attention_block_2, 9-Name
        operator = Spec[0]
        exp_size = Spec[1]
        filters = Spec[2]
        kernel_size = Spec[3]
        ...
        padding = Spec[5]
        activation = Spec[6]
        # Attention_block = Spec[7]
        name = Spec[9]
        drop_out = Spec[10]
        
        if Spec[7] != None:
            Spec[7] = Attention_block_1
        if Spec[8] != None:
            Spec[8] = Attention_block_2
        
        if Spec[0] == "Conv":
            x = Conv2D(filters=Spec[2], kernel_size=Spec[3], strides=(Spec[4], Spec[4]), padding=Spec[5], 
                       name=Spec[9], kernel_regularizer=L2(0.00005), kernel_initializer=initializer)(x)
            x = BatchNormalization()(x)
            if Spec[6] =="HS":
                x = HardSwish(name=Spec[9]+"hardswish")(x)
            elif Spec[6] =="RE":
                x = ReLU(name=Spec[9]+"relu")(x)
        elif Spec[0] == 'DWconv':
            x = DepthwiseConv2D(kernel_size=Spec[3], strides=(Spec[4],Spec[4]), padding=Spec[5], activation=Spec[6],
                                name=Spec[9], kernel_initializer=initializer)(x)
        elif Spec[0] == "MBblock":
            x = MBConv(x, exp_size=Spec[1], filters=Spec[2], kernel_size=Spec[3], strides=Spec[4], padding=Spec[5],
                       activation=Spec[6], Attention_block_1=Spec[7], name=Spec[9], dropout=Spec[10], initializer=initializer)
        elif Spec[0] == "CSPblock":
            x = CSP_block(x, exp_size=Spec[1], filters=Spec[2], kernel_size=Spec[3], strides=Spec[4], padding=Spec[5],
                          activation=Spec[6], Attention_block_1=Spec[7], Attention_block_2=Spec[8], name=Spec[9], dropout=Spec[10], initializer=initializer)
