...
def CSP_block(x, exp_size, filters, kernel_size, strides=1, padding='same', activation='HS',
              Attention_block_1=None, Attention_block_2=None, name='CSP', dropout=1, initializer='random_uniform'):
    # --------Part 1--------------------
    ...
    part_1 = MBConv(x,
                    ...
                    Attention_block=Attention_block_1,
                    padding=padding,
    ...
    # Attenetion Block
    if Attention_block_2 == None:
        pass
    elif Attention_block_2 == 'SE':
        part_2 = SE_block(part_2, name=name)
    elif Attention_block_2 == 'CBAM':
        part_2 = CBAM_block(part_2, name=name)
    elif Attention_block_2 == 'CA':
        part_2 = CA_block(part_2, name=name)
    ...
    #     pass
    # elif Attention_block_2 == 'SE':
    #     part_1 = SE_block(x, name=name)
    # elif Attention_block_2 == 'CBAM':
    #     part_1 = CBAM_block(x, name=name)
    # elif Attention_block_2 == 'CA':
    #     part_1 = CA_block(x, name=name)

    return x


def MBConv(x, exp_size, filters, kernel_size, strides=1, padding='same', activation='HS',
        Attention_block_1=None, name='', dropout=1, initializer='random_uniform'):
    _, _, _, c = np.shape(x)
    # ----------------------part_1--------------------------------------
    # Conv1x1
    ...
    # SE block
    if Attention_block_1 == None:
        pass
    elif Attention_block_1 == 'SE':
        part_1 = SE_block(part_1, name=name)
    elif Attention_block_1 == 'CBAM':
        part_1 = CBAM_block(part_1, name=name)
    elif Attention_block_1 == 'CA':
    ...
    # --------------------part_2_resblock------------------------------
    # if r:
    # StochasticDepth 類似dropout，不同在於dropout是針對一部分的unit，而StochasticDepth是針對整個resblock
    if c == filters and strides == 1:
        if dropout != None:
            x = StochasticDepth(dropout, name=name +
                                "_stochastic_depth")([x, part_1])
        else:
...
def SE_block(x, r=4, name=''):
    ...
    x_attention = Dense(C/r, name=name+"_SE_Dense1",
                        kernel_regularizer=L2(0.00005))(x_attention)
    x_attention = BatchNormalization(name=name+"_SE_bn1")(x_attention)
    x_attention = HardSwish(name=name+"_SE_hardswish")(x_attention)
    x_attention = Dense(C, name=name+"_SE_Dense2",
                        kernel_regularizer=L2(0.00005))(x_attention)
    ...
    x_attention = reshape(x_attention, shape=(-1, 1, 1, C),
                        name=name+"_SE_Reshape")
...
def CBAM_block(x, r=2, name=''):
    ...
    channel_attention = HardSigmoid(
        name=name+"_CBAM_ADD_hardsigmoid")(channel_attention)

    # -----------------------------------channel_attention與x相乘-----------------------------------------------------------
    channel_attention = Reshape(target_shape=(
        1, 1, C), name=name+"_CBAM_Reshape")(channel_attention)
...
def CA_block(x, r=2, name=""):
    ...
    x_h = AdaptiveAveragePooling2D(
        (h, 1), name=name+'_Coordinate_AdaptiveAveragePooling_h')(x)  # H x 1 x C
    x_h = Permute((3, 2, 1))(x_h)
    x_w = AdaptiveAveragePooling2D(
        (1, w), name=name+'_Coordinate_AdaptiveAveragePooling_w')(x)  # 1 x W x C
    x_w = Permute((3, 1, 2))(x_w)

    # 1 x (W + H) x C/r
    xy = Concatenate(name=name+"_Coordinate_Concatenate")([x_h, x_w])
    xy = Permute((2, 3, 1), name=name+"_Coordinate_Permute")(xy)
    xy = Conv2D(filters=mip, kernel_size=1, strides=(1, 1),
                padding='same', name=name+"_Coordinate_Conv1")(xy)
    xy = BatchNormalization(name=name+"_Coordinate_BatchNormalization")(xy)
    xy = HardSigmoid(name=name+"_Coordinate_HardSigmoid1")(xy)

    x_h, x_w = split(xy, num_or_size_splits=2, axis=2,
                     name=name+"_Coordinate_split")
    # x_w = Permute((2,1,3))(x_w)

    a_h = Conv2D(filters=c, kernel_size=1, strides=(1, 1),
                 padding='same', name=name+"_Coordinate_Conv2")(x_h)
    a_h = HardSigmoid(name=name+"_Coordinate_HardSigmoid2")(a_h)
    a_w = Conv2D(filters=c, kernel_size=1, strides=(1, 1),
                 padding='same', name=name+"_Coordinate_Conv3")(x_w)
